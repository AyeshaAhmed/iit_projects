...
class Index(object):
    ...
    def create_champion_index(self, index, threshold=10):
        ...
        for term in index.keys():
            champ_vect[term] = sorted(index[term], key=lambda item:item[1], reverse=True)[:threshold]
        return champ_vect
        pass
    ...
    def create_tfidf_index(self, docs, doc_freqs):
        ...
        for index,doc in enumerate(docs):
            for term in set(doc):
                tf = (math.log10(doc.count(term))+1)
                idf = math.log10(len(docs)/doc_freqs[term])
                tf_idf[term].append([index,tf*idf])
        return tf_idf
        pass
    ...
    def count_doc_frequencies(self, docs):
        ...
        """
        ###TODO
        termCount = defaultdict(lambda: 0)
        for doc in docs:
            for term in set(doc):
                termCount[term] += 1
        return termCount
    ...
    def query_to_vector(self, query_terms):
        ...
        for t in query_terms:
            if t in self.doc_freqs.keys():
                idf = math.log10(len(self.documents)/self.doc_freqs[t])
                query_vect[t]=idf
        return query_vect
        pass
    ...
    def search_by_cosine(self, query_vector, index, doc_lengths):
        ...
        """
        ###TODO
        doc_vect = defaultdict(lambda: {})
        for term in query_vector.keys():
            for results in index[term]:
                doc_vect[results[0]][term] = results[1]
        cosines = []
        for key,value in doc_vect.items():
            doc_priority = 0
            for term, weight in value.items():
                doc_priority += query_vector[term]*weight
            cosines.append((key, doc_priority/doc_lengths[key]))
        return sorted(cosines, key=lambda item:item[1], reverse=True)
        pass
    ...
    def search(self, query, use_champions=False):
        ...
        query_vect = self.query_to_vector(q)
        resultlist = []
        if use_champions is True:
            resultlist = self.search_by_cosine(query_vect, self.create_champion_index(self.index), self.doc_lengths)
        else: 
            resultlist = self.search_by_cosine(query_vect, self.index, self.doc_lengths)
        return resultlist
        pass
