...
def tokenize(document):
    ...
    """
    ###TODO
    return re.findall('\w+', document.lower()) #\w=a word character [a-zA-Z0-9_]; +=1 or more
    pass
...
def sort_by_num_postings(words, index):
    ...
    """
    ###TODO
    return sorted(words, key=lambda v : len(index[v]))
    pass
...
def search(index, query):
    ...
    """
    ###TODO
    res = sort_by_num_postings(tokenize(query.lower()), index)
    k = 1
    ans = index[res[0]]
    while k<len(res):
        ans = intersect(ans, index[res[k]]) 
        k+=1
    return ans
    pass
