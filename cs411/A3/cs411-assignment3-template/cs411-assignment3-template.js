...
// Ayesha Ahmed
//
// cs411 assignment 3 (Fall 2016) - spline curves
//
// BROWSER: Google Chrome 53.0.2785.143
//			
//	NOTE: Animation doesn't work on Firefox 49.0.1
//
/////////////////////////////////////////////////////////////////////////////////////////
...
function tensionUp() {
  ...
  intrPts = []; 
  for (var nCtrlPts = 0; nCtrlPts < (ctrlPts.length+1); nCtrlPts++){
      if (nCtrlPts > 3){ 
...
{
///////////////////////////////////////////////////////////////////////////////////////
// MY CODE (TO USE CARDINAL SPLINES)
  for(var u=0; u<=1; u+=uStep){
	//calc cardinals
	var c1 = (-1*tension)*Math.pow(u, 3) + (2*tension)*Math.pow(u, 2)     + (-1*tension)*u;
	var c2 = (2-tension)*Math.pow(u, 3)  + (tension-3)*Math.pow(u, 2)     + 1;
	var c3 = (tension-2)*Math.pow(u, 3)  + (3-(2*tension))*Math.pow(u, 2) + tension*u;
	var c4 = tension*Math.pow(u, 3)      + (-1*tension)*Math.pow(u, 2);
	//calc x and y
	var x = c1*pk_n1.x + c2*pk.x + c3*pk_p1.x + c4*pk_p2.x;
	var y = c1*pk_n1.y + c2*pk.y + c3*pk_p1.y + c4*pk_p2.y;
	//push x and y
	intrPts.push(x);
	intrPts.push(y);
  }
